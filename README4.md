...
https://archive.ics.uci.edu/ml/index.php) or in its [LIBSVM
  ...
  - Put UCI datasets with LIBSVM format into the folder `\genUCIdatafun\UCIdataorg`;
  - Run *genUCIdata.m*, then the expanded UCI data in MAT format will be generated in the folder `UCIdata` (For demonstration purpose, "mpg_scale.txt" and "housing_scale.txt" are left in the folder `\genUCIdatafun\UCIdataorg`).

***Step 4***. Run the scripts for generating the corresponding results in paper according the following table:
...
***Step 5***. Run the scripts for generating the corresponding results in the supplementary materials according the following table:
...
<table>
	...
	<tr>
		...
		<td rowspan="2">`Test1_fixed_lambda`</td>
		<td><i>Test_NALM_random</td>
		<td>prob=[1 2], flag_tol=0, tol=1e-6, alpha_vec=[0.9, 0.5, 0.1]</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td rowspan="2">`Test1_fixed_lambda`</td>
		<td><i>Test_NALM_random</td>
		<td>prob=[3 4], flag_tol=0, tol=1e-6, alpha_vec=[0.9, 0.5, 0.1]</td>
	</tr>
	<tr>
	        <td><i>Test_PSG_random</td>
	...
	<tr>
		...
		<td rowspan="2">`Test1_fixed_lambda`</td>
		<td><i>Test_NALM_random</td>
		<td>prob=[5:7], flag_tol=0, tol=1e-8, alpha_vec=[0.9, 0.5, 0.1]</td>
	</tr>
	<tr>
