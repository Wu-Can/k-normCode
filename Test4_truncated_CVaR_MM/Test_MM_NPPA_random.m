%% ===============================================================================
%% Test the MM+N-PPA for the nonconvex truncated CVaR-based model with random data
%% Input:
...
for ii = 1:len_m
    ...
    for jj = 1:lenlam
        ...
        result(jj+(ii-1)*lenlam,10) = info.iterPPA;
        result(jj+(ii-1)*lenlam,11) = info.iterSSN;
        result(jj+(ii-1)*lenlam,12) = info.time;
        result(jj+(ii-1)*lenlam,13) = obj;
    end
end
